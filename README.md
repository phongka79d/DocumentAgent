...
- `frontend/` contains a Vite React TypeScript application with an Axios API client, typed document and chat/evidence API contracts, reusable upload/document display/chat input components, routed upload and document list pages, compact navigation, responsive/accessibility styling, and a shared file validation helper.
...
Plan 14 Batch02 completed reusable frontend chat input controls. `DocumentSelector` can load or receive document list data, allows only `ready` documents to be selected, exposes typed ready-document validation helpers, and shows loading/error/empty/no-ready states. `ChatBox` provides a controlled question textarea, trimmed non-empty validation, validation feedback, and a loading/disabled submit state. The full routed chat page and evidence UI remain planned for later Plan 14 batches.
...
- Frontend layer: `frontend/src/` is a routed React application with an Axios client, typed document upload/list/detail API helpers, typed chat/evidence API helpers, upload and document list pages, reusable document display/chat input components, compact navigation, responsive/accessibility styling, and shared file validation.
...
- `frontend/src/api/agentRuns.ts` exposes typed `getAgentRunEvidence` through the backend `GET /api/agent-runs/{agent_run_id}/evidence` route, along with safe evidence API error mapping. It does not call the agent logs endpoint.
...
The upload and document list screens are mounted and navigable. Chat, evidence viewer, and agent log frontend screens are not implemented yet; Plan 14 currently provides chat/evidence API contracts and reusable selector/input controls, but not the assembled chat route.
...
`frontend/src/pages/EvidenceViewerPage.tsx` is a route-ready direct evidence screen that reads an `agentRunId` route parameter, handles invalid/loading/error states, and renders the shared verified/rejected evidence panel. `App.tsx` route and navigation integration remains a separate frontend routing task.
...
- The frontend now mounts routed upload and document list screens with compact navigation and responsive/accessibility styling. Chat/evidence API clients, reusable controls and display components, the assembled chat page, and a route-ready direct evidence page exist. Chat/evidence route registration, retrieval UI, and agent-log UI remain planned.
