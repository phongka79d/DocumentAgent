...
- `backend/app/agents/answer_agent.py`: Agent 3 citation formatting, verified/rejected evidence validation, visible chunk-ID blocking, self-check normalization, and ready-state enforcement helpers.
...
- `backend/tests/test_answer_agent.py`: Agent 3 schema, citation/evidence contract, prompt-rule, and deterministic self-check readiness tests.
...
| `SHOPAIKEY_CHAT_MODEL` | Yes for backend chat completion calls | Chat model used by backend ShopAIKey chat completion callers, including graph extraction and planned Agent 3 answer/self-check generation. | `backend/app/core/config.py` |
...
- Agent 1 shared schemas exist in `backend/app/agents/schemas.py`; the retrieval agent callable exists in `backend/app/agents/retrieval_agent.py` and logs retrieval steps through `backend/app/services/agent_log_service.py`. Agent 2 verification schemas, reusable prompt rules, the backend-only verification callable, deterministic evidence safety checks, and Agent 2 success/failed step logging exist. Agent 3 answer schemas, citation/evidence validation helpers, answer and self-check prompt contracts, and deterministic self-check readiness enforcement helpers exist, but the `run_answer_agent` runtime callable, Agent 3 step logging, insufficient-evidence runtime behavior, and full agent run workflow orchestration are still planned.
