...
- `backend/app/agents/answer_agent.py`: Agent 3 internal callable, controlled answer error, input normalization, verified/rejected evidence lookup, deterministic insufficient-evidence output, verified-only provider payload/message construction, citation formatting, verified/rejected evidence validation, visible chunk-ID blocking, self-check normalization, and ready-state enforcement helpers.
...
- `backend/tests/test_answer_agent.py`: Agent 3 callable, input normalization, insufficient-evidence, provider payload, schema, citation/evidence contract, prompt-rule, and deterministic self-check readiness tests.
...
- Agent 1 shared schemas exist in `backend/app/agents/schemas.py`; the retrieval agent callable exists in `backend/app/agents/retrieval_agent.py` and logs retrieval steps through `backend/app/services/agent_log_service.py`. Agent 2 verification schemas, reusable prompt rules, the backend-only verification callable, deterministic evidence safety checks, and Agent 2 success/failed step logging exist. Agent 3 answer schemas, citation/evidence validation helpers, answer and self-check prompt contracts, deterministic self-check readiness enforcement helpers, `run_answer_agent` validation, deterministic insufficient-evidence runtime behavior, and verified-only provider payload construction exist, but Agent 3 provider draft parsing, runtime self-check execution, Agent 3 step logging, and full agent run workflow orchestration are still planned.
