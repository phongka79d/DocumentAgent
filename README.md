...
- `backend/app/agents/schemas.py`: shared Agent 1 retrieval input, candidate, and output Pydantic schemas.
...
- `backend/app/agents/schemas.py` and `backend/app/agents/retrieval_agent.py` before changing Agent 1 workflow contracts or retrieval-agent behavior.
...
- Agent 1 shared schemas exist in `backend/app/agents/schemas.py`; the retrieval agent callable exists in `backend/app/agents/retrieval_agent.py` and logs retrieval steps through `backend/app/services/agent_log_service.py`. Full agent run workflow orchestration is still planned.
