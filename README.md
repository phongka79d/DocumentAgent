...
- `backend/app/agents/schemas.py`: shared Agent 1 retrieval input, candidate, and output Pydantic schemas, plus Agent 2 verification input/output schemas.
- `backend/app/agents/prompts.py`: reusable Agent 2 verification prompt rules and required output-key metadata.
...
- `backend/app/agents/verification_agent.py`: backend-only Agent 2 callable for verification input validation, empty-candidate missing-information output, compact ShopAIKey chat requests, strict JSON parsing, Pydantic output validation, deterministic candidate membership checks, quote fidelity checks, duplicate filtering, contradiction/missing-information adjustment, and final output shape preservation.
...
- `backend/app/agents/schemas.py`, `backend/app/agents/prompts.py`, `backend/app/agents/retrieval_agent.py`, and `backend/app/agents/verification_agent.py` before changing Agent 1 workflow contracts, Agent 2 verification contracts, prompt rules, retrieval-agent behavior, or verification-agent behavior.
