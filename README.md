...
- `backend/app/agents/verification_agent.py`: backend-only Agent 2 callable for verification input validation, empty-candidate missing-information output, compact ShopAIKey chat requests, strict JSON parsing, and Pydantic output validation.
...
- Agent 1 shared schemas exist in `backend/app/agents/schemas.py`; the retrieval agent callable exists in `backend/app/agents/retrieval_agent.py` and logs retrieval steps through `backend/app/services/agent_log_service.py`. Agent 2 verification schemas, reusable prompt rules, and the backend-only verification callable exist, but deterministic evidence safety checks, verification logging, and full agent run workflow orchestration are still planned.
