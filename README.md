...
- `frontend/` contains a Vite React TypeScript shell with an Axios API client, typed document API contracts, reusable upload/document display components, an upload page with recent-document feedback, a dedicated document list page with manual status refresh, a shared file validation helper, and React Router available for planned routing.
...
- `frontend/package.json` includes `react-router-dom` for planned upload and document list routing.
...
The upload page and document list page are not mounted in `App.tsx` yet, and navigation remains planned for later Plan 13 batches.
...
- The frontend is currently still mounted as a placeholder shell rather than a routed working upload/chat UI. Typed document API helpers, response contracts, reusable upload/document display components, shared file validation, `UploadDocumentPage.tsx`, and `DocumentListPage.tsx` now exist, but route wiring in `App.tsx`, navigation, and chat UI remain planned.
