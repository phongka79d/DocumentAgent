...
- `frontend/` contains a Vite React TypeScript shell with an Axios API client, typed document API contracts, reusable upload/document display components, an upload page with recent-document feedback, a shared file validation helper, and React Router available for planned routing.
...
The upload page is not mounted in `App.tsx` yet, and the separate document list page plus navigation remain planned for later Plan 13 batches.
