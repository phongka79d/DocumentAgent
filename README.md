...
- `backend/app/schemas/agent_runs.py`: Plan 12 agent-run evidence and logs response contracts for future evidence/log endpoints.
...
- `backend/app/services/chat_service.py`: service-layer chat persistence preparation, selected document ownership validation, user/assistant message persistence helpers, and controlled chat errors for later API mapping.
- `backend/app/services/agent_run_service.py`: agent run lifecycle helpers, Agent 2 evidence extraction from persisted steps, ordered log response mapping, and controlled agent-run errors for later API mapping.
...
Only some of these tables are currently used by service code. Document upload/list/detail uses `documents`; processing and indexing use `document_chunks`. Graph helper contracts can read chunks and write `document_entities` and `document_relationships`; the current graph builder writes structural document-section and section-chunk relationships, de-duplicated extracted entities, chunk-entity links, valid entity-entity links, and strong-overlap chunk-chunk links. Agent 1 retrieval, Agent 2 verification, and Agent 3 answer/self-check can write `agent_steps` through the backend agent log service. Chat session/message helpers, agent run lifecycle helpers, selected-document ownership lookup, and owned evidence/log lookup now exist in the service layer. The internal LangGraph workflow creates a running `agent_runs` row, invokes Agent 1 -> Agent 2 -> Agent 3, marks success or failure, and returns response-ready answer fields. Public chat/evidence/log routes and chat-message persistence around public ask requests are not yet wired into runtime workflows.
...
- `backend/app/schemas/chat.py` and `backend/app/schemas/agent_runs.py` before changing planned chat, evidence, or agent-log API response contracts.
...
- Agent 1 shared schemas exist in `backend/app/agents/schemas.py`; the retrieval agent callable exists in `backend/app/agents/retrieval_agent.py` and logs retrieval steps through `backend/app/services/agent_log_service.py`. Agent 2 verification schemas, reusable prompt rules, the backend-only verification callable, deterministic evidence safety checks, and Agent 2 success/failed step logging exist. Agent 3 answer schemas, citation/evidence validation helpers, answer and self-check prompt contracts, deterministic self-check readiness enforcement helpers, `run_answer_agent` validation, deterministic insufficient-evidence runtime behavior, verified-only provider payload construction, provider draft parsing, citation/evidence enforcement, runtime self-check execution, controlled self-check failure handling, output-shape normalization, Agent 3 step logging, and safe log-insertion failure visibility exist. `backend/app/agents/graph.py` now provides the internal LangGraph state and callable contract for Agent 1 -> Agent 2 -> Agent 3 execution, creates and updates the `agent_runs` lifecycle row, returns Agent 3 answer fields, and treats Agent 2 missing-information output as a successful insufficient-evidence path when Agent 3 handles it. Service-layer chat persistence, selected-document ownership validation, and evidence/log lookup helpers exist, but route-level failure mapping, chat-message persistence around the public ask route, and public chat/evidence/log API wiring are still planned.
- `backend/app/schemas/chat.py` and `backend/app/schemas/agent_runs.py` define planned public response/request contracts for chat ask, evidence, and logs. `backend/app/services/chat_service.py` and `backend/app/services/agent_run_service.py` provide service-layer persistence and lookup helpers for those contracts, but the corresponding API routers are still planned.
