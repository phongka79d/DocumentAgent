...
- `frontend/` contains a Vite React TypeScript application with an Axios API client, typed document and chat/evidence API contracts, reusable upload/document/chat/evidence components, routed upload, document list, chat, and direct evidence pages, compact navigation, responsive/accessibility styling, and a shared file validation helper.
...
React Router is mounted through `BrowserRouter`. `App.tsx` exposes `/upload`, `/documents`, `/chat`, and `/evidence/:agentRunId`; root and unknown paths redirect to `/upload`. Primary navigation links to Upload, Documents, and Chat without full-page reloads.
...
The upload, document list, chat, and direct evidence pages are mounted in `App.tsx` with compact route-aware navigation. Styling supports visible focus and active states, disabled/busy feedback, structured state panels, long-content wrapping, and narrow layouts down to the 320px minimum viewport.
...
- The frontend now mounts routed upload, document list, chat, and direct evidence screens with compact navigation and responsive/accessibility styling. Direct retrieval UI and agent-log/debug UI remain out of scope for Plan 14.
