...
- `backend/app/agents/schemas.py`, `backend/app/agents/prompts.py`, and `backend/app/agents/retrieval_agent.py` before changing Agent 1 workflow contracts, Agent 2 verification contracts, prompt rules, or retrieval-agent behavior.
...
- Agent 1 shared schemas exist in `backend/app/agents/schemas.py`; the retrieval agent callable exists in `backend/app/agents/retrieval_agent.py` and logs retrieval steps through `backend/app/services/agent_log_service.py`. Agent 2 verification schemas and reusable prompt rules exist, but the Agent 2 runtime callable, verification logging, and full agent run workflow orchestration are still planned.
